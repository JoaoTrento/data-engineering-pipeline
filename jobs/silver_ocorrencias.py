...
def get_dados_ocorrencias_bronze():
...
def get_dados_aeroportos_silver():
    aeroportos = spark.table("anac.silver.aeroportos").select("icao_id", "categoria", "aeroporto")

    return aeroportos

def junta_dados_ocorrencias_aeroportos(silver: DataFrame, aeroportos: DataFrame):
    silver = silver.join(
        aeroportos,
...
def add_qtd_aeronaves_envolvidas(silver: DataFrame):
    qtd_aeronaves = (
        silver
...
def salva_ocorrencias_silver(silver: DataFrame):
    silver.write \
        .format("delta") \
        .mode("overwrite") \
        .saveAsTable(
